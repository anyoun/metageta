	...
	public class TVDBPlugin : IMGTaggingPlugin, IMGPluginBase {
		...
		private int? GetSeriesID(MGFile file) {
			...
			if (!m_SeriesNameDictionary.TryGetValue(seriesName, out seriesID)) {
				seriesID = LookupSeriesID(seriesName);
		...
		private int? LookupSeriesID(string seriesName) {
			var searchResult = m_tvdbHandler.SearchSeries(seriesName);
			if (searchResult.Count == 0) {
				log.DebugFormat("Couldn't find series: \"{0}\".", seriesName);

				//Try again without " (YEAR)" at end of series title
				var match = Regex.Match(seriesName, @"^(.*?)\s*\(\d{4}\)$");
				if (match.Success)
					return LookupSeriesID(match.Groups[1].Value);

				return null;
			} else if (searchResult.Count == 1) {
				log.DebugFormat("Found series: \"{0}\" -> \"{1}\".", seriesName, searchResult.Single().SeriesName);
				return searchResult.Single().Id;
			} else {
