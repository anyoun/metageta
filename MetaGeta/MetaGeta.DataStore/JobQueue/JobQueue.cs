	...
	public class JobQueue : IDisposable, INotifyPropertyChanged {
		...
		public JobQueue(bool designMode) {
			if (!designMode) {
				...
				EnqueueAction(null, null, null);
				EnqueueAction(null, null, null);
				EnqueueAction(null, null, null);
				m_AllActionItems[0].Status.Start();
				m_AllActionItems[0].Status.Done(true);
				m_AllActionItems[1].Status.Start();
				m_AllActionItems[1].Status.ProgressPct = .3;
			}
		}

		public IList<Job> ActionItems {
			get {
		...
		public Job CurrentItem {
			get {
				lock (m_ActionWaitingQueue)
					return m_AllActionItems.FirstOrDefault(j => j.Status.IsRunning);
			}
		}
		public int NotDoneCount {
			get {
				lock (m_ActionWaitingQueue)
					return m_AllActionItems.Count(j => !j.Status.IsDone);
			}
		}
		...
		private void OnJobStatusChanged() {
			OnPropertyChanged("NotDoneCount");
