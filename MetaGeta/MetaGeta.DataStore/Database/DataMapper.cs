...
public class DataMapper : IDataMapper {
	...
	private readonly ThreadLocal<DbConnection> m_Connection;
	private readonly object m_AllConnectionsLock = new object();
	private readonly List<DbConnection> m_AllConnections = new List<DbConnection>();
	...
	public DataMapper(string filename) {
		m_FileName = filename;
		m_Connection = new ThreadLocal<DbConnection>(() => {
			...
			conn.Open();
			lock (m_AllConnectionsLock)
				m_AllConnections.Add(conn);
			return conn;
		});
	}

	private DbConnection Connection { get { return m_Connection.Value; } }
	...
	public void Close() {
		//Must be synchronous...
		lock (m_AllConnectionsLock)
			foreach (var conn in m_AllConnections)
				conn.Close();
	}
